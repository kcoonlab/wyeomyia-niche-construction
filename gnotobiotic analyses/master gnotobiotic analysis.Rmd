...
unsat_removed_physeq<-readRDS("./gno_final_unsat_removed")
...
# take a look at stocks, unfortunately field cololonized stock dna extraction failed, plotting only lab stocks 
temp<-prune_samples(sample_data(unsat_removed_physeq)$gno_type =="stock" & sample_data(unsat_removed_physeq)$gno_trt !="un_field stock",unsat_removed_physeq)
mdf_prep <- prep_mdf(temp, remove_na = TRUE)
color_obj <- microshades::create_color_dfs(mdf_prep, group_level= "Phylum", subgroup_level= "Genus", selected_groups = c("Actinobacteriota", "Bacteroidota","Proteobacteria"))

# pick your favorite 5 phyla to look at! I use the information from my initial summarizing above to inform which 5 to choose... or perhaps you are particularly interested in a given group for some reason

mdf <- color_obj$mdf
cdf <- color_obj$cdf

new_groups <- extend_group(mdf, cdf, "Phylum", "Genus", "Proteobacteria", existing_palette = "micro_purple", new_palette = "micro_cvd_purple", n_add = 5)
...
facet_labels <- as_labeller(c(
  "cnv stock" = "Conventional","lab stock" = "Laboratory", "mom stock"= "Parental"))

plot_diff <- plot + scale_y_continuous(labels = scales::percent, expand = expansion(0)) +
  theme(legend.position = "none")  +
  theme(axis.text.x = element_text(size= 10)) +
  facet_nested(~gno_trt, scales = "free_x", labeller= facet_labels) +
  # theme(axis.text.x = element_text(size= 10)) + 
  theme(axis.text.x = element_blank() ) +
  theme(plot.margin = margin(6,20,6,6)) + xlab("Glycerol preserved homogenate") + ggtitle("") + theme(plot.title = element_text(size=11))

stacked_bacteria<-plot_grid(plot_diff, legend,  rel_widths = c(1, .25))

# ggsave("supp figure stocks.pdf",plot=stacked_bacteria, path = "c:/Users/aldoa/OneDrive/Desktop/",device="pdf",dpi=320, height=5,width=10)
###################
```

```{r}

#for purposes of viz maybe we do not need  the really small groups
...
Rename metadata for table generation
```{r}
temp_meta<-temp_meta %>%
...
Fixing metadata in physeq object and plotting ordination
```{r message=FALSE, warning=FALSE}
...
```{r}
# some values unstable at perm = 1000 (default) significance calls more stable at perm = 10000
set.seed(1)
###
pair_adonis<-pairwiseAdonis::pairwise.adonis(clr_dist_matrix,sample_data(s_clr)$gno_trt, reduce = NULL, perm = 10000)
...
```{r}
new<-read.csv("./temp field cfu survey.csv")

library(ggplot2)
library(cowplot)

plot_new<-ggplot(new,aes(x= Treatment  , y=log, fill= Treatment)) + geom_boxplot(lwd=0.5, outlier.shape = NA) + 
  ...
  panel.grid.major = element_line(colour = "grey70", size = 0.1), panel.grid.minor = element_blank(), axis.text.x=element_blank()) +  xlab("Mosquito background") + ylab("log(CFU/mL)")
...
ggsave("cfu_early_field.pdf",plot=plot_new, path = "./",device="pdf",dpi=320, height=6,width=4)
