...
read in file
```{r}
seq_object = readRDS("./final LP phyloseq")
```
...
x<-ggplot(beta_merge, aes(x= paste, y= Beta.Stock)) + geom_boxplot(lwd=0.5, outlier.shape = NA) + 
  # scale_y_continuous(limits = c(30, 60))+
  geom_point(position=position_jitter(width = 0.1, height = NULL, seed = NA) ,shape = 21, size = 1, fill= "black") +
    theme_cowplot()+
   theme(
    legend.position = 0,
  text=element_text(size=13.5),
  axis.text= element_text(size=13.5),
  panel.grid.major = element_line(colour = "grey70", size = 0.1), panel.grid.minor = element_blank(), axis.text.x=element_blank()) +  xlab("") + ylab(expression(beta*"-Diversity Relative to Starting Community (a.u.)"))+
...
ggsave("beta larval passaging.pdf",plot=x, path = "./",device="pdf",dpi=320, height=4,width=5)
...
```{r}
df<-as.data.frame(as.matrix(sample_data(seq_object)))
df_water_cfu<-df %>% filter(Water_Larvae=="water")
df_water_cfu$Density.CFU.mL.<-as.numeric(df_water_cfu$Density.CFU.mL.)


plot_new<-ggplot(df_water_cfu,aes(x= Treatment, y=log10(Density.CFU.mL.), fill= Treatment)) + geom_boxplot(lwd=0.5, outlier.shape = NA) + 
  geom_point(position=position_jitter(width = 0.1, height = NULL, seed = NA) ,shape = 21, size = 1.5, fill= "black") +
    theme_cowplot()+
   theme(
    legend.position = NULL,
  text=element_text(size=13.5),
  axis.text= element_text(size=13.5),
  panel.grid.major = element_line(colour = "grey70", size = 0.1), panel.grid.minor = element_blank(), axis.text.x=element_blank()) +
  scale_x_discrete(guide = guide_axis(n.dodge = 2)) + xlab("Mosquito Background") + ylab("log(CFU/mL)")
...
LABELS 
ggsave("cfu_larval_passaging.pdf",plot=plot_new, path = "c:/Users/aldoa/OneDrive/Desktop/",device="pdf",dpi=320, height=6,width=4)
```
...
for(focal_asv in list) {
  ...
  binomial_model_temp<-lme4::glmer(formula = proportion_survived ~ Abundance + (1|Trial),
                   data = focal_asv_df,
                   weights= Larvae_Start,
                   family = binomial)
...
merged_2 %>% filter(FDR<0.05 & `Model Coefficient` > 0)
```
